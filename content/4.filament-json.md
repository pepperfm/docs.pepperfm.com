...
- Raw JSON strings are decoded when they contain valid JSON, so database values like
...
`JsonColumn` follows Filament's own state resolution first, then normalizes the value for JSON rendering.
That means `state()`, `getStateUsing()`, `default()`, relationship state, separators, and Filament state caching
...
button and modal blade components from Filament documentation: [Core Concepts - Blade Components](https://filamentphp.com/docs/5.x/components/overview)
::
```php
use PepperFM\FilamentJson\Columns\JsonColumn;
...
```
![image](/content/filament-json/img.webp)
...
```
![image](/content/filament-json/img_1.webp)
...
```
![image](/content/filament-json/img_2.webp)
...
```
![image](/content/filament-json/img_3.webp)
...
```
![image](/content/filament-json/img_4.webp)
